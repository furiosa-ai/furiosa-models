...
class YOLOv5PreProcessor(PreProcessor):
    @staticmethod
    def __call__(
        images: Sequence[np.ndarray], color_format: str
...
class YOLOv5PostProcessor(PostProcessor):
    ...
    def __call__(
        ...
        Args:
            ...
            contexts (Dict[str, Any]): A configuration for each image generated by the preprocessor.
                For example, it could be the reduction ratio of the image, the actual image width and height.
            conf_thres (float, optional): Confidence score threshold. The default to 0.25
