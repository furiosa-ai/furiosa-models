...
async def test_mlcommons_ssd_resnet34_perf():
    ...
    with await detector.create_session() as sess:
        ...
        result = detector.inference(
            sess, detector.load_image(test_image_path), confidence_threshold=0.3
        )
        print(f"result={result}")
        assert len(result) == 2, "ssd_resnet34 output shape must be 2"
        assert (
            result[0].predicted_class == "cat"
        ), f"wrong predicted_class: {result[0].predicted_class}, expected cat"
        assert (
            result[1].predicted_class == "cat"
        ), f"wrong predicted_class: {result[1].predicted_class}, expected cat"
        assert (
            np.sum(np.abs(result[0].boundingbox.numpy() - true_bbox[0])) < 1e-3
        ), f"bbox is different from expected value"
        assert (
            np.sum(np.abs(result[1].boundingbox.numpy() - true_bbox[1])) < 1e-3
        ), f"bbox is different from expected value"
        assert (
